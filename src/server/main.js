...
const HIGH_LATIN_CHAR = /[\u0081-\u00FF\u02C6\u2019\u201C\u201D\u2013\u2014\u20AC\u0161\u017D\u017E\u0178\u201A\u0192\u201E\u2020\u2021\u02DC\u2122\u203A\u00AB\u00BB]/,
  ...
  LITERAL_ESCAPE_PATTERN = /\\u([0-9a-fA-F]{4})/g,
  CP1252_TO_BYTE = new Map([[0x20AC,0x80],[0x201A,0x82],[0x0192,0x83],[0x201E,0x84],[0x2026,0x85],[0x2020,0x86],[0x2021,0x87],[0x02C6,0x88],[0x2030,0x89],[0x0160,0x8A],[0x2039,0x8B],[0x0152,0x8C],[0x017D,0x8E],[0x2018,0x91],[0x2019,0x92],[0x201C,0x93],[0x201D,0x94],[0x2022,0x95],[0x2013,0x96],[0x2014,0x97],[0x02DC,0x98],[0x2122,0x99],[0x0161,0x9A],[0x203A,0x9B],[0x0153,0x9C],[0x017E,0x9E],[0x0178,0x9F]]);
function repairMojibakeRun(run) {
  try {
    const bytes = Uint8Array.from(run, character => {
      const code = character.charCodeAt(0);
      return CP1252_TO_BYTE.get(code) ?? code;
    }),
      text = UTF8_REPAIR_DECODER.decode(bytes);
    return text.length < run.length ? text : null;
  } catch { return null; }
}
function repairVisibleText(value) {
  if (typeof value !== "string" || value.length < 2) return value;
  let out = "", index = 0, changed = false;
  while (index < value.length) {
    if (!HIGH_LATIN_CHAR.test(value[index])) { out += value[index++]; continue; }
    let end = index;
    while (end < value.length && HIGH_LATIN_CHAR.test(value[end])) end++;
    const run = value.slice(index, end),
      repaired = repairMojibakeRun(run);
    if (repaired !== null) { out += repaired; changed = true; }
    else out += run;
    index = end;
  }
  const fixEscapes = (text) => text.replace(LITERAL_ESCAPE_PATTERN, (_, hex) => {
    const code = Number.parseInt(hex, 16);
    return code >= 0x20 && code <= 0xffff ? String.fromCharCode(code) : `\\u${hex}`;
  });
  const repairedText = fixEscapes(changed ? out : value);
  return changed && repairedText !== value && repairedText.search(HIGH_LATIN_CHAR) !== -1
    ? repairVisibleText(repairedText)
    : repairedText;
}
function repairCommandText(value, depth = 0) {
  if (typeof value === "string") return repairVisibleText(value);
  if (!value || typeof value !== "object" || depth > 4) return value;
  if (Array.isArray(value)) return value.map(item => repairCommandText(item, depth + 1));
  const repaired = {};
  for (const [key, item] of Object.entries(value)) repaired[key] = repairCommandText(item, depth + 1);
  return repaired;
}
function normalizeCommands(result) {
  return result.commands.map(command => {
    ...
    return repairCommandText(tool ? { ...command, tool } : command);
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/plugins") {
    try {
      ...
      const body = await readJson(req, MAX_PLUGIN_DOCUMENT_BYTES + MAX_PLUGIN_STYLES_BYTES + 4 * 1024);
